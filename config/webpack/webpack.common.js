...
var getCommonConfiguration = function (_a) {
    ...
    if (!isDevelopment) {
        var releaseName = "".concat(process.env.npm_package_name, "@").concat(process.env.npm_package_version);
        console.debug("[SENTRY ".concat(platform.toUpperCase(), "] Release: ").concat(releaseName));
        console.debug("[SENTRY ".concat(platform.toUpperCase(), "] Assets Path: ").concat(platform === 'desktop' ? './desktop/dist/www/**/*.{js,map}' : './dist/**/*.{js,map}'));
    }
    return {
        ...
        plugins: __spreadArray(__spreadArray(__spreadArray(__spreadArray(__spreadArray(__spreadArray([
        ...
        ], false), (isDevelopment ? [] : [new mini_css_extract_plugin_1.default()]), true), (isDevelopment
            ? []
            : [
                // eslint-disable-next-line @typescript-eslint/no-unsafe-call
                (0, webpack_plugin_1.sentryWebpackPlugin)({
                    authToken: process.env.SENTRY_AUTH_TOKEN,
                    org: 'expensify',
                    project: 'app',
                    release: {
                        name: "".concat(process.env.npm_package_name, "@").concat(process.env.npm_package_version),
                    },
                    sourcemaps: {
            ...
            ]), true), (process.env.ANALYZE_BUNDLE === 'true' ? [new webpack_bundle_analyzer_1.BundleAnalyzerPlugin()] : []), true),
        module: {
            rules: [
                {
                    test: /\.m?js$/,
                ...
                {
                    test: /\.css$/i,
                    use: isDevelopment ? ['style-loader', 'css-loader'] : [mini_css_extract_plugin_1.default.loader, 'css-loader'],
                },
                {
                    test: /\.(woff|woff2)$/i,
                    type: 'asset',
        ...
        resolve: {
            alias: {
                ...
                // eslint-disable-next-line @typescript-eslint/naming-convention
                '@selectors': path_1.default.resolve(__dirname, '../../src/selectors/'),
            },
