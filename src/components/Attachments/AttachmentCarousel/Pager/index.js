...
function AttachmentCarouselPager(_a, ref) {
    var items = _a.items, activeAttachmentID = _a.activeAttachmentID, initialPage = _a.initialPage, setShouldShowArrows = _a.setShouldShowArrows, onPageSelected = _a.onPageSelected, onSwipeDown = _a.onSwipeDown, reportID = _a.reportID, onAttachmentError = _a.onAttachmentError;
    ...
    var pagerItems = (0, react_1.useMemo)(function () { return items.map(function (item, index) { return ({ source: item.source, previewSource: item.previewSource, index: index, isActive: index === activePageIndex, attachmentID: item.attachmentID }); }); }, [activePageIndex, items]);
    ...
    return (<AttachmentCarouselPagerContext_1.default.Provider value={contextValue}>
            <react_native_gesture_handler_1.GestureDetector gesture={nativeGestureHandler}>
                <AnimatedPagerView pageMargin={40} offscreenPageLimit={1} onPageScroll={pageScrollHandler} onPageSelected={onPageSelected} style={styles.flex1} initialPage={initialPage} animatedProps={animatedProps} ref={pagerRef}>
                    {carouselItems}
                </AnimatedPagerView>
