 ...
 */
var successListener = function (event, preferredLocale) {
    var token = event.detail.authorization.id_token;
    (0, Session_1.beginAppleSignIn)(token, preferredLocale);
};
...
function AppleSignInDiv(_a) {
    ...
    react_1.default.useEffect(function () {
        document.addEventListener('AppleIDSignInOnSuccess', function (event) { return successListener(event, preferredLocale); });
        document.addEventListener('AppleIDSignInOnFailure', failureListener);
        return function () {
            document.removeEventListener('AppleIDSignInOnSuccess', function (event) { return successListener(event, preferredLocale); });
