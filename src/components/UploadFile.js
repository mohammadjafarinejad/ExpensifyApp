...
function UploadFile(_a) {
    ...
    var handleFileUpload = function (files) {
        ...
        if (acceptedFileTypes.length > 0) {
            var filesExtensions_1 = new Set(files.map(function (file) { var _a; return (0, FileUtils_1.splitExtensionFromFileName)((_a = file === null || file === void 0 ? void 0 : file.name) !== null && _a !== void 0 ? _a : '').fileExtension.toLowerCase(); }));
            if (acceptedFileTypes.every(function (element) { return !filesExtensions_1.has(element); })) {
                setError(translate('attachmentPicker.notAllowedExtension'));
                return;
            }
        }
        var uploadedFilesNames = new Set(uploadedFiles.map(function (uploadedFile) { return uploadedFile.name; }));
        var newFilesToUpload = files.filter(function (file) { return !uploadedFilesNames.has(file.name); });
