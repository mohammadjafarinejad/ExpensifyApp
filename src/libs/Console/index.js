...
var logPatternsToIgnore = ["merge called for key: ".concat(ONYXKEYS_1.default.LOGS)];
/**
...
function logMessage(args) {
    var message = args
        ...
        return (0, SafeString_1.default)(arg);
    })
        .join(' ');
...
function createLog(text) {
    ...
    try {
        ...
        if (result !== undefined) {
            return [
                { time: time, level: CONST_1.default.DEBUG_CONSOLE.LEVELS.INFO, message: "> ".concat(text), extraData: '' },
                { time: time, level: CONST_1.default.DEBUG_CONSOLE.LEVELS.RESULT, message: (0, SafeString_1.default)(result), extraData: '' },
            ];
        }
        return [{ time: time, level: CONST_1.default.DEBUG_CONSOLE.LEVELS.INFO, message: "> ".concat(text), extraData: '' }];
    }
    catch (error) {
        return [
            { time: time, level: CONST_1.default.DEBUG_CONSOLE.LEVELS.ERROR, message: "> ".concat(text), extraData: '' },
            { time: time, level: CONST_1.default.DEBUG_CONSOLE.LEVELS.ERROR, message: "Error: ".concat(error.message), extraData: '' },
        ];
    }
}
/**
