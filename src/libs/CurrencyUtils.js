 ...
 * @param currency - IOU currency
 */
function convertToDisplayString(amountInCents, currency, shouldUseLocalCurrencySymbol) {
    if (amountInCents === void 0) { amountInCents = 0; }
    if (currency === void 0) { currency = CONST_1.default.CURRENCY.USD; }
    if (shouldUseLocalCurrencySymbol === void 0) { shouldUseLocalCurrencySymbol = false; }
    ...
    if (shouldUseLocalCurrencySymbol) {
        var currencySymbol = getCurrencySymbol(currencyWithFallback);
        if (currencySymbol) {
            var formattedNumber = (0, NumberFormatUtils_1.format)(IntlStore_1.default.getCurrentLocale(), convertedAmount, {
                style: 'decimal',
