...
function isOpenDomainSplitAction(action) {
    return action.type === CONST_1.default.NAVIGATION.ACTION_TYPE.OPEN_DOMAIN_SPLIT;
}
...
function RootStackRouter(options) {
    ...
    return __assign(__assign({}, stackRouter), { getStateForAction: function (state, action, configOptions) {
            ...
            if (isOpenDomainSplitAction(action)) {
                return (0, GetStateForActionHandlers_1.handleOpenDomainSplitAction)(state, action, configOptions, stackRouter);
            }
