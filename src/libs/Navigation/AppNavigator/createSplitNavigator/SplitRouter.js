...
function adaptStateIfNecessary(_a) {
    ...
    if (!navigationRef_1.default.isReady()) {
        Log_1.default.warn('[src/libs/Navigation/AppNavigator/createSplitNavigator/SplitRouter.ts] NavigationRef is not ready. Returning the original state without adaptation.');
    }
    var isNarrowLayout = (0, getIsNarrowLayout_1.default)();
    var rootState = navigationRef_1.default.isReady() ? navigationRef_1.default.getRootState() : undefined;
...
function isPushingSidebarOnCentralPane(state, action, options) {
    var isSidebarAction = (action.type === CONST_1.default.NAVIGATION.ACTION_TYPE.PUSH || action.type === CONST_1.default.NAVIGATION.ACTION_TYPE.NAVIGATE) && action.payload.name === options.sidebarScreen;
    if (!isSidebarAction) {
        return false;
    }
    var sidebarExists = state.routes.some(function (route) { return route.name === options.sidebarScreen; });
...
function shouldPopEntireNavigator(state, action) {
    return action.type === CONST_1.default.NAVIGATION.ACTION_TYPE.GO_BACK && !(0, getIsNarrowLayout_1.default)() && state.routes.length === 2;
}
function SplitRouter(options) {
    ...
    return __assign(__assign({}, stackRouter), { getStateForAction: function (state, action, configOptions) {
            ...
            if (shouldPopEntireNavigator(state, action)) {
                var stateAfterPop = stackRouter.getStateForAction(state, native_1.StackActions.pop(), configOptions);
                return stackRouter.getStateForAction(stateAfterPop, native_1.StackActions.pop(), configOptions);
            }
