...
var activeSpans_1 = require("@libs/telemetry/activeSpans");
...
function getReportRHPActiveRoute() {
    ...
    if (!navigationRef_1.default.isReady()) {
        Log_1.default.warn('[src/libs/Navigation/Navigation.ts] NavigationRef is not ready. Returning empty string.');
        return '';
    }
...
function navigate(route, options) {
    ...
    if (route.startsWith('r/') || route.startsWith('search/r/')) {
        var reportIDMatch = route.match(/^(?:search\/)?r\/(\w+)/);
        if (reportIDMatch === null || reportIDMatch === void 0 ? void 0 : reportIDMatch.at(1)) {
            var reportID = reportIDMatch.at(1);
            var spanName = route.startsWith('r/') ? '/r/*' : '/search/r/*';
            (0, activeSpans_1.startSpan)("".concat(CONST_1.default.TELEMETRY.SPAN_OPEN_REPORT, "_").concat(reportID), {
                name: spanName,
                op: CONST_1.default.TELEMETRY.SPAN_OPEN_REPORT,
                attributes: {
                    reportID: reportID,
                },
            });
...
function popToSidebar() {
    ...
    if ((topRoute === null || topRoute === void 0 ? void 0 : topRoute.name) !== RELATIONS_1.SPLIT_TO_SIDEBAR[currentRouteName]) {
        var params = currentRoute.name === NAVIGATORS_1.default.WORKSPACE_SPLIT_NAVIGATOR || currentRoute.name === NAVIGATORS_1.default.DOMAIN_SPLIT_NAVIGATOR ? __assign({}, lastRoute === null || lastRoute === void 0 ? void 0 : lastRoute.params) : undefined;
...
function removeReportScreen(reportIDSet) {
    isNavigationReady().then(function () {
        var _a;
        (_a = navigationRef_1.default.current) === null || _a === void 0 ? void 0 : _a.dispatch(function (state) {
            var routes = state === null || state === void 0 ? void 0 : state.routes.filter(function (route) {
                var _a;
                if (route.name === SCREENS_1.default.REPORT && route.params && 'reportID' in route.params) {
                    return !reportIDSet.has((_a = route.params) === null || _a === void 0 ? void 0 : _a.reportID);
                }
                return true;
            });
            return native_1.CommonActions.reset(__assign(__assign({}, state), { routes: routes, index: routes.length < state.routes.length ? state.index - 1 : state.index }));
...
function isValidateLoginFlow() {
    var state = navigationRef_1.default.getRootState();
    var currentFocusedRoute = (0, core_1.findFocusedRoute)(state);
    return (currentFocusedRoute === null || currentFocusedRoute === void 0 ? void 0 : currentFocusedRoute.name) === SCREENS_1.default.VALIDATE_LOGIN;
}
...
exports.default = {
    ...
    removeReportScreen: removeReportScreen,
    ...
    isValidateLoginFlow: isValidateLoginFlow,
};
