...
exports.getActivePaymentType = getActivePaymentType;
...
function hasExpensifyPaymentMethod(fundList, bankAccountList, shouldIncludeDebitCard) {
    ...
    var validBankAccount = Object.values(bankAccountList).some(function (bankAccountJSON) {
        ...
        return (bankAccount.getPendingAction() !== CONST_1.default.RED_BRICK_ROAD_PENDING_ACTION.DELETE &&
            ...
            (bankAccount === null || bankAccount === void 0 ? void 0 : bankAccount.getCountry()) === CONST_1.default.COUNTRY.US);
    });
...
function getPaymentMethodDescription(accountType, account, translate, bankCurrency) {
    var _a, _b, _c;
    if (account) {
        if (accountType === CONST_1.default.PAYMENT_METHODS.PERSONAL_BANK_ACCOUNT && 'accountNumber' in account) {
            return "".concat(bankCurrency ? "".concat(bankCurrency, " ").concat(CONST_1.default.DOT_SEPARATOR, " ") : '').concat(translate('paymentMethodList.accountLastFour'), " ").concat((_a = account.accountNumber) === null || _a === void 0 ? void 0 : _a.slice(-4));
        }
        if (accountType === CONST_1.default.PAYMENT_METHODS.BUSINESS_BANK_ACCOUNT && 'accountNumber' in account) {
            return "".concat(translate('paymentMethodList.accountLastFour'), " ").concat((_b = account.accountNumber) === null || _b === void 0 ? void 0 : _b.slice(-4));
        }
        if (accountType === CONST_1.default.PAYMENT_METHODS.DEBIT_CARD && 'cardNumber' in account) {
            return "".concat(translate('paymentMethodList.cardLastFour'), " ").concat((_c = account.cardNumber) === null || _c === void 0 ? void 0 : _c.slice(-4));
        }
    }
    return '';
}
 ...
 */
function formatPaymentMethods(bankAccountList, fundList, styles, translate) {
    ...
    Object.values(bankAccountList).forEach(function (bankAccount) {
        ...
        combinedPaymentMethods.push(__assign(__assign({}, bankAccount), { description: getPaymentMethodDescription(bankAccount === null || bankAccount === void 0 ? void 0 : bankAccount.accountType, bankAccount.accountData, translate, bankAccount.bankCurrency), icon: icon, iconSize: iconSize, iconHeight: iconHeight, iconWidth: iconWidth, iconStyles: iconStyles }));
    });
    Object.values(fundList).forEach(function (card) {
        ...
        combinedPaymentMethods.push(__assign(__assign({}, card), { description: getPaymentMethodDescription(card === null || card === void 0 ? void 0 : card.accountType, card.accountData, translate), icon: icon, iconSize: iconSize, iconHeight: iconHeight, iconWidth: iconWidth, iconStyles: iconStyles }));
    });
 ...
 */
var selectPaymentType = function (params) {
    var event = params.event, iouPaymentType = params.iouPaymentType, triggerKYCFlow = params.triggerKYCFlow, policy = params.policy, onPress = params.onPress, currentAccountID = params.currentAccountID, currentEmail = params.currentEmail, hasViolations = params.hasViolations, isASAPSubmitBetaEnabled = params.isASAPSubmitBetaEnabled, isUserValidated = params.isUserValidated, confirmApproval = params.confirmApproval, iouReport = params.iouReport;
    ...
    if (iouPaymentType === CONST_1.default.IOU.PAYMENT_TYPE.EXPENSIFY || iouPaymentType === CONST_1.default.IOU.PAYMENT_TYPE.VBBA) {
        ...
        triggerKYCFlow({ event: event, iouPaymentType: iouPaymentType });
    ...
    if (iouPaymentType === CONST_1.default.IOU.REPORT_ACTION_TYPE.APPROVE) {
        if (confirmApproval) {
            ...
            (0, IOU_1.approveMoneyRequest)(iouReport, policy, currentAccountID, currentEmail, hasViolations, isASAPSubmitBetaEnabled, true);
        }
        return;
    }
 ...
 */
function getActivePaymentType(paymentMethod, activeAdminPolicies, latestBankItems) {
    var isPaymentMethod = Object.values(CONST_1.default.PAYMENT_METHODS).includes(paymentMethod);
    var shouldSelectPaymentMethod = isPaymentMethod || !(0, isEmpty_1.default)(latestBankItems);
    var selectedPolicy = activeAdminPolicies.find(function (activePolicy) { return activePolicy.id === paymentMethod; });
    var paymentType;
    ...
    return {
        ...
        shouldSelectPaymentMethod: shouldSelectPaymentMethod,
    };
}
