 ...
 */
function hasBillingDisputePending(billingDisputePending) {
...
function shouldShowPreTrialBillingBanner(introSelected, firstDayFreeTrial, lastDayFreeTrial) {
    var _a;
...
function shouldShowDiscountBanner(hasTeam2025Pricing, subscriptionPlan, firstDayFreeTrial, lastDayFreeTrial, userBillingFundID) {
    var _a;
...
function getEarlyDiscountInfo(firstDayFreeTrial) {
 ...
 */
function hasRetryBillingError(retryBillingFailed) {
    return !!retryBillingFailed;
}
/**
 * @returns Whether the retry billing was successful.
 */
function isRetryBillingSuccessful(retryBillingSuccessful) {
    return !!retryBillingSuccessful;
}
/**
 * @returns The subscription status.
 */
function getSubscriptionStatus(stripeCustomerId, retryBillingSuccessful, billingDisputePending, retryBillingFailed) {
 ...
 */
function hasSubscriptionRedDotError(stripeCustomerId, retryBillingSuccessful, billingDisputePending, retryBillingFailed) {
    var _a, _b;
    return (_b = (_a = getSubscriptionStatus(stripeCustomerId, retryBillingSuccessful, billingDisputePending, retryBillingFailed)) === null || _a === void 0 ? void 0 : _a.isError) !== null && _b !== void 0 ? _b : false;
}
/**
 * @returns Whether there is a subscription green dot info.
 */
function hasSubscriptionGreenDotInfo(stripeCustomerId, retryBillingSuccessful, billingDisputePending, retryBillingFailed) {
    var _a;
    return ((_a = getSubscriptionStatus(stripeCustomerId, retryBillingSuccessful, billingDisputePending, retryBillingFailed)) === null || _a === void 0 ? void 0 : _a.isError) === false;
}
/**
 * Calculates the remaining number of days of the workspace owner's free trial before it ends.
 */
function calculateRemainingFreeTrialDays(lastDayFreeTrial) {
    if (!lastDayFreeTrial) {
        return 0;
    }
 ...
 */
function getFreeTrialText(translate, policies, introSelected, firstDayFreeTrial, lastDayFreeTrial) {
 ...
 */
function isUserOnFreeTrial(firstDayFreeTrial, lastDayFreeTrial) {
 ...
 */
function hasUserFreeTrialEnded(lastDayFreeTrial) {
    if (!lastDayFreeTrial) {
        return false;
    }
    var currentDate = new Date();
 ...
 */
function doesUserHavePaymentCardAdded(userBillingFundID) {
...
function shouldCalculateBillNewDot(canDowngrade) {
    if (canDowngrade === void 0) { canDowngrade = false; }
    return canDowngrade && (0, PolicyUtils_1.getOwnedPaidPolicies)(allPolicies, currentUserAccountID).length === 1;
}
function getSubscriptionPrice(plan, preferredCurrency, privateSubscriptionType, hasTeam2025Pricing) {
...
function getSubscriptionPlanInfo(translate, subscriptionPlan, privateSubscriptionType, preferredCurrency, isFromComparisonModal, hasTeam2025Pricing) {
    var priceValue = getSubscriptionPrice(subscriptionPlan, preferredCurrency, privateSubscriptionType, hasTeam2025Pricing);
    ...
    if (subscriptionPlan === CONST_1.default.POLICY.TYPE.TEAM) {
        var subtitle = translate('subscription.yourPlan.customPricing');
        var note = translate('subscription.yourPlan.asLowAs', { price: price });
        if (hasTeam2025Pricing) {
            if (isFromComparisonModal) {
                ...
                subtitle = translate('subscription.yourPlan.pricePerMemberMonth', { price: price });
                note = undefined;
            }
        }
        return {
    ...
    return {
        ...
        note: translate('subscription.yourPlan.asLowAs', { price: price }),
        benefits: [
