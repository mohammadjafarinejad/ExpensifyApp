...
function sortTaxRates(taxRates, localeCompare) {
    var sortedTaxRates = Object.values(taxRates).sort(function (a, b) { return localeCompare(a.name, b.name); });
...
function getTaxRatesSection(_a) {
    var policy = _a.policy, searchValue = _a.searchValue, localeCompare = _a.localeCompare, _b = _a.selectedOptions, selectedOptions = _b === void 0 ? [] : _b, transaction = _a.transaction;
    var policyRatesSections = [];
    var taxes = (0, TransactionUtils_1.transformedTaxRates)(policy, transaction);
    var sortedTaxRates = sortTaxRates(taxes, localeCompare);
    var selectedOptionNames = new Set(selectedOptions.map(function (selectedOption) { return selectedOption.modifiedName; }));
    var enabledTaxRates = sortedTaxRates.filter(function (taxRate) { return !taxRate.isDisabled; });
    var enabledTaxRatesNames = new Set(enabledTaxRates.map(function (tax) { return tax.modifiedName; }));
    var enabledTaxRatesWithoutSelectedOptions = enabledTaxRates.filter(function (tax) { return tax.modifiedName && !selectedOptionNames.has(tax.modifiedName); });
