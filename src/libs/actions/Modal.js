 ...
 */
function closeTop(topModalOffset) {
    if (closeModals.length === 0) {
        return;
    }
    // Add bounds to the offset to ensure it's not out of bounds and use topmost modal by default.
    var startFromTopMostModal = topModalOffset === undefined ? -1 : Math.max(Math.min(-topModalOffset - 1, -1), -closeModals.length);
    var modalCloseCallback = closeModals.splice(startFromTopMostModal, 1).at(0);
    if (onModalClose) {
        modalCloseCallback === null || modalCloseCallback === void 0 ? void 0 : modalCloseCallback(isNavigate);
        return;
    }
    modalCloseCallback === null || modalCloseCallback === void 0 ? void 0 : modalCloseCallback();
}
/**
 * Close modal in other parts of the app
 */
function close(onModalCloseCallback, isNavigating, shouldCloseAllModals, topModalOffset) {
    if (isNavigating === void 0) { isNavigating = true; }
    if (shouldCloseAllModals === void 0) { shouldCloseAllModals = false; }
    if (topModalOffset === void 0) { topModalOffset = 0; }
    if (closeModals.length === 0) {
        onModalCloseCallback === null || onModalCloseCallback === void 0 ? void 0 : onModalCloseCallback();
        return;
    }
