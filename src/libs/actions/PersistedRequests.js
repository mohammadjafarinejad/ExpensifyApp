...
exports.onInitialization = onInitialization;
...
function onInitialization(callbackFunction) {
...
react_native_onyx_1.default.connectWithoutView({
    ...
    callback: function (val) {
        ...
        if (pendingSaveOperations.length > 0) {
            Log_1.default.info("[PersistedRequests] Processing pending save operations, size: ".concat(pendingSaveOperations.length), false);
            var requests = __spreadArray(__spreadArray([], persistedRequests, true), pendingSaveOperations, true);
        ...
        if (!isInitialized && persistedRequests.length > 0) {
            Log_1.default.info('[PersistedRequests] Triggering initialization callback', false);
...
function save(requestToPersist) {
    ...
    if (!isInitialized) {
        Log_1.default.info('[PersistedRequests] Queueing request until initialization completes', false);
...
function processNextRequest() {
    ...
    ongoingRequest = persistedRequests.length > 0 ? ((_a = persistedRequests.at(0)) !== null && _a !== void 0 ? _a : null) : null;
    // Create a new array without the first element
    var newPersistedRequests = persistedRequests.slice(1);
