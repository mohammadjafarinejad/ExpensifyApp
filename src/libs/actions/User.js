...
exports.updateStatusDraftCustomClearAfterDate = updateStatusDraftCustomClearAfterDate;
...
exports.requestUnlockAccount = requestUnlockAccount;
...
function triggerNotifications(onyxUpdates) {
    onyxUpdates.forEach(function (update) {
        ...
        reportActions.forEach(function (action) { return action && (0, Report_1.showReportActionNotification)(reportID, action); });
    });
}
 ...
 *
 */
function updateStatusDraftCustomClearAfterDate(date) {
    react_native_onyx_1.default.set(ONYXKEYS_1.default.STATUS_DRAFT_CUSTOM_CLEAR_AFTER_DATE, date);
}
/**
...
function setNameValuePair(name, value, revertedValue, shouldRevertValue) {
    if (shouldRevertValue === void 0) { shouldRevertValue = true; }
    var parameters = {
        name: name,
        value: value,
    };
    ...
    // @ts-expect-error - will be solved in https://github.com/Expensify/App/issues/73830
    var failureData = shouldRevertValue
        ? [
            {
                onyxMethod: react_native_onyx_1.default.METHOD.MERGE,
                key: name,
...
function requestUnlockAccount() {
    ...
    API.write(types_1.WRITE_COMMANDS.REQUEST_UNLOCK_ACCOUNT, params);
}
