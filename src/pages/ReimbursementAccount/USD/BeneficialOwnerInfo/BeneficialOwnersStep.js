...
function BeneficialOwnersStep(_a) {
    ...
    var submit = function () {
        ...
        var beneficialOwners = beneficialOwnerKeys.map(function (ownerKey) {
            return beneficialOwnerFields.reduce(function (acc, fieldName) {
                acc[fieldName] = reimbursementAccountDraft ? (0, SafeString_1.default)(reimbursementAccountDraft["beneficialOwner_".concat(ownerKey, "_").concat(fieldName)]) : undefined;
                return acc;
            }, {});
        });
    ...
    return (<InteractiveStepWrapper_1.default wrapperID={BeneficialOwnersStep.displayName} shouldEnablePickerAvoiding={false} shouldEnableMaxHeight headerTitle={translate('beneficialOwnerInfoStep.companyOwner')} handleBackButtonPress={handleBackButtonPress} shouldShowOfflineIndicatorInWideScreen={currentUBOSubStep === SUBSTEP.UBOS_LIST} startStepIndex={5} stepNames={CONST_1.default.BANK_ACCOUNT.STEP_NAMES}>
            {currentUBOSubStep === SUBSTEP.IS_USER_UBO && (<YesNoStep_1.default title={translate('beneficialOwnerInfoStep.doYouOwn25percent', { companyName: companyName })} description={translate('beneficialOwnerInfoStep.regulationRequiresUsToVerifyTheIdentity')} submitButtonStyles={[styles.mb0]} defaultValue={isUserUBO} onSelectedValue={handleNextUBOSubstep}/>)}

            {currentUBOSubStep === SUBSTEP.IS_ANYONE_ELSE_UBO && (<YesNoStep_1.default title={translate('beneficialOwnerInfoStep.doAnyIndividualOwn25percent', { companyName: companyName })} description={translate('beneficialOwnerInfoStep.regulationRequiresUsToVerifyTheIdentity')} submitButtonStyles={[styles.mb0]} defaultValue={isAnyoneElseUBO} onSelectedValue={handleNextUBOSubstep}/>)}
            ...
            {currentUBOSubStep === SUBSTEP.ARE_THERE_MORE_UBOS && (<YesNoStep_1.default title={translate('beneficialOwnerInfoStep.areThereMoreIndividualsWhoOwn25percent', { companyName: companyName })} description={translate('beneficialOwnerInfoStep.regulationRequiresUsToVerifyTheIdentity')} submitButtonStyles={[styles.mb0]} onSelectedValue={handleNextUBOSubstep} defaultValue={false}/>)}
