...
function AnimatedEmptyStateBackground() {
    ...
    var illustrationWidth = CONST_1.default.EMPTY_STATE_BACKGROUND.ASPECT_RATIO * CONST_1.default.EMPTY_STATE_BACKGROUND.WIDE_SCREEN.IMAGE_HEIGHT; // or whatever your SVG's natural width is
    var maxBackgroundWidth = variables_1.default.sideBarWidth + illustrationWidth;
    // If window width is greater than the max background width, repeat the background image
    var numberOfRepeats = windowWidth > maxBackgroundWidth ? Math.ceil(windowWidth / illustrationWidth) : 1;
    ...
    return (<react_native_1.View style={StyleUtils.getReportWelcomeBackgroundContainerStyle()}>
            <react_native_reanimated_1.default.View style={[StyleUtils.getReportWelcomeBackgroundImageStyle(shouldUseNarrowLayout), animatedStyles]} entering={react_native_reanimated_1.FadeIn}>
                {Array.from({ length: numberOfRepeats }).map(function (_, index) { return (<ImageSVG_1.default 
