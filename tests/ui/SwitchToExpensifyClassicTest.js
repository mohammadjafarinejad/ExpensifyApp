...
function navigateToSetting() {
    var hintText = TestHelper.translateLocal('sidebarScreen.buttonMySettings');
...
function navigateToExpensifyClassicFlow() {
    var hintText = TestHelper.translateLocal('exitSurvey.goToExpensifyClassic');
...
describe('Switch to Expensify Classic flow', function () {
    ...
    test('Should navigate to exit survey reason page', function () {
        signInAppAndEnterTestFlow(true).then(function () {
            expect(react_native_1.screen.getAllByText(TestHelper.translateLocal('exitSurvey.reasonPage.subtitle')).at(0)).toBeOnTheScreen();
        });
    });
});
