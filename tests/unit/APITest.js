...
describe('APITests', function () {
    ...
    test('Write request with supportal insufficient permissions (411) suppresses retry, applies failureData, and triggers modal', function () {
        var _a;
        var xhr = jest.spyOn(HttpUtils_1.default, 'xhr').mockResolvedValue({ jsonCode: 411, message: 'You are not authorized to take this action when support logged in.' });
        var onyxUpdateSpy = jest.spyOn(Onyx, 'update');
        var onyxSetSpy = jest.spyOn(Onyx, 'set');
        var failureData = [
            {
        ...
        return Onyx.multiSet((_a = {},
            _a[ONYXKEYS_1.default.SESSION] = { authToken: 'anyToken', authTokenType: CONST_1.default.AUTH_TOKEN_TYPES.SUPPORT },
            _a[ONYXKEYS_1.default.NETWORK] = { isOffline: false },
            _a))
            .then(function () {
            // @ts-expect-error - will be solved in https://github.com/Expensify/App/issues/73830
            API.write('MockCommand', {}, { failureData: failureData });
            return (0, waitForNetworkPromises_1.default)();
        })
            .then(waitForBatchedUpdates_1.default)
            .then(function () {
            ...
            var failureApplied = onyxUpdateSpy.mock.calls.some(function (_a) {
                var updates = _a[0];
                if (!Array.isArray(updates)) {
                    return false;
                }
                return updates.some(function (u) { return (u === null || u === void 0 ? void 0 : u.key) === ONYXKEYS_1.default.ONBOARDING_ERROR_MESSAGE_TRANSLATION_KEY && (u === null || u === void 0 ? void 0 : u.value) === 'onboarding.errorSelection'; });
            });
            expect(failureApplied).toBe(true);
            // Supportal permission middleware should have set the modal payload via Onyx.set
            var supportalModalSet = onyxSetSpy.mock.calls.some(function (_a) {
                var key = _a[0], payload = _a[1];
                if (key !== ONYXKEYS_1.default.SUPPORTAL_PERMISSION_DENIED) {
                    return false;
                }
                if (typeof payload !== 'object' || payload === null) {
                    return false;
                }
                return payload.command === 'MockCommand';
            });
