...
describe('DateUtils', function () {
    ...
    it('should return a date object with the formatted datetime when calling getLocalDateFromDatetime', function () {
        var localDate = DateUtils_1.default.getLocalDateFromDatetime(LOCALE, timezone, datetime);
        expect((0, date_fns_tz_1.format)(localDate, CONST_1.default.DATE.FNS_TIMEZONE_FORMAT_STRING, { timeZone: timezone })).toEqual('2022-11-07T00:00:00Z');
    });
    it('should fallback to current date when getLocalDateFromDatetime is failing', function () {
        var localDate = DateUtils_1.default.getLocalDateFromDatetime(LOCALE, 'InvalidTimezone', undefined);
    ...
    it('should update timezone if automatic and selected timezone do not match', function () { return __awaiter(void 0, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    _a.sent();
                    result = DateUtils_1.default.getCurrentTimezone({ selected: 'Europe/London', automatic: true });
    ...
    describe('getStatusUntilDate', function () {
        var currentTimeZone = 'America/Los_Angeles';
        var inputTimeZoneNY = 'America/New_York';
        var inputTimeZoneParis = 'Europe/Paris';
        var inputTimeZoneTokyo = 'Asia/Tokyo';
        beforeEach(function () {
            jest.useFakeTimers();
        ...
        it('returns empty string when input date is empty', function () {
            expect(DateUtils_1.default.getStatusUntilDate('', inputTimeZoneNY, currentTimeZone)).toBe('');
        });
        it('returns "Until h:mm a" when input and current timezone are same', function () {
            var nowInTZ = (0, date_fns_tz_1.toZonedTime)(new Date(), currentTimeZone);
            var targetTime = (0, date_fns_1.set)(nowInTZ, { hours: 15, minutes: 34, seconds: 0, milliseconds: 0 });
            var inputDateStr = (0, date_fns_tz_1.format)(targetTime, CONST_1.default.DATE.FNS_DATE_TIME_FORMAT_STRING, { timeZone: currentTimeZone });
            var result = DateUtils_1.default.getStatusUntilDate(inputDateStr, currentTimeZone, currentTimeZone);
            var expectedLabel = (0, date_fns_tz_1.format)(targetTime, CONST_1.default.DATE.LOCAL_TIME_FORMAT, { timeZone: currentTimeZone });
            expect(result).toBe("Until ".concat(expectedLabel));
        });
        it('returns "Until tomorrow" when end of day is in the same timezone', function () {
            var nowInCurrent = (0, date_fns_tz_1.toZonedTime)(new Date(), currentTimeZone);
            var endOfTodayCurrent = (0, date_fns_1.endOfDay)(nowInCurrent);
            var inputDateStrNY = (0, date_fns_tz_1.format)(endOfTodayCurrent, CONST_1.default.DATE.FNS_DATE_TIME_FORMAT_STRING, { timeZone: inputTimeZoneNY });
            var result = DateUtils_1.default.getStatusUntilDate(inputDateStrNY, inputTimeZoneNY, inputTimeZoneNY);
            expect(result).toBe('Until tomorrow');
        });
        it('returns "Until h:mm a" for later today in a different timezone', function () {
            var targetTimeLA = (0, date_fns_1.set)((0, date_fns_tz_1.toZonedTime)(new Date(), currentTimeZone), { hours: 15, minutes: 34, seconds: 0, milliseconds: 0 });
            var inputDateStrNY = (0, date_fns_tz_1.format)(targetTimeLA, CONST_1.default.DATE.FNS_DATE_TIME_FORMAT_STRING, { timeZone: inputTimeZoneNY });
            var result = DateUtils_1.default.getStatusUntilDate(inputDateStrNY, inputTimeZoneNY, currentTimeZone);
            var date = (0, date_fns_tz_1.fromZonedTime)(inputDateStrNY, inputTimeZoneNY);
            var converted = (0, date_fns_tz_1.toZonedTime)(date, currentTimeZone);
            var expectedLabel = (0, date_fns_tz_1.format)(converted, CONST_1.default.DATE.LOCAL_TIME_FORMAT, { timeZone: currentTimeZone });
            expect(result).toBe("Until ".concat(expectedLabel));
        });
        it('returns "Until MM-dd h:mm a" for future date within the same year in a different timezone', function () {
            var twoDaysLaterLA = (0, date_fns_1.addDays)((0, date_fns_1.set)((0, date_fns_tz_1.toZonedTime)(new Date(), currentTimeZone), { hours: 15, minutes: 0, seconds: 0, milliseconds: 0 }), 2);
            var inputDateStrParis = (0, date_fns_tz_1.format)(twoDaysLaterLA, CONST_1.default.DATE.FNS_DATE_TIME_FORMAT_STRING, { timeZone: inputTimeZoneParis });
            var result = DateUtils_1.default.getStatusUntilDate(inputDateStrParis, inputTimeZoneParis, currentTimeZone);
            var date = (0, date_fns_tz_1.fromZonedTime)(inputDateStrParis, inputTimeZoneParis);
            var converted = (0, date_fns_tz_1.toZonedTime)(date, currentTimeZone);
            var expectedLabel = (0, date_fns_tz_1.format)(converted, "".concat(CONST_1.default.DATE.SHORT_DATE_FORMAT, " ").concat(CONST_1.default.DATE.LOCAL_TIME_FORMAT), { timeZone: currentTimeZone });
            expect(result).toBe("Until ".concat(expectedLabel));
        });
        it('returns "Until MM-dd h:mm a" when "until today" crosses into next day in current timezone', function () {
            var endOfTodayTokyo = (0, date_fns_1.endOfDay)((0, date_fns_tz_1.toZonedTime)(new Date(), inputTimeZoneTokyo));
            var inputDateStrTokyo = (0, date_fns_tz_1.format)(endOfTodayTokyo, CONST_1.default.DATE.FNS_DATE_TIME_FORMAT_STRING, { timeZone: inputTimeZoneTokyo });
            var result = DateUtils_1.default.getStatusUntilDate(inputDateStrTokyo, inputTimeZoneTokyo, currentTimeZone);
            var date = (0, date_fns_tz_1.fromZonedTime)(inputDateStrTokyo, inputTimeZoneTokyo);
            var converted = (0, date_fns_tz_1.toZonedTime)(date, currentTimeZone);
            var expectedLabel = (0, date_fns_tz_1.format)(converted, "".concat(CONST_1.default.DATE.SHORT_DATE_FORMAT, " ").concat(CONST_1.default.DATE.LOCAL_TIME_FORMAT), { timeZone: currentTimeZone });
            expect(result).toBe("Until ".concat(expectedLabel));
        });
        it('returns "Until yyyy-MM-dd h:mm a" for a date in a different year across timezones', function () {
            var laFutureDateStr = '2026-01-02 09:15:00';
            var inputDateStrTokyo = (0, date_fns_tz_1.format)((0, date_fns_tz_1.fromZonedTime)(laFutureDateStr, currentTimeZone), CONST_1.default.DATE.FNS_DATE_TIME_FORMAT_STRING, { timeZone: inputTimeZoneTokyo });
            var result = DateUtils_1.default.getStatusUntilDate(inputDateStrTokyo, inputTimeZoneTokyo, currentTimeZone);
            var date = (0, date_fns_tz_1.fromZonedTime)(inputDateStrTokyo, inputTimeZoneTokyo);
            var converted = (0, date_fns_tz_1.toZonedTime)(date, currentTimeZone);
            var expectedLabel = (0, date_fns_tz_1.format)(converted, "".concat(CONST_1.default.DATE.FNS_FORMAT_STRING, " ").concat(CONST_1.default.DATE.LOCAL_TIME_FORMAT), { timeZone: currentTimeZone });
            expect(result).toBe("Until ".concat(expectedLabel));
        });
    });
});
