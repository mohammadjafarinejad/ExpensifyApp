...
describe('EmojiTest', function () {
    ...
    it('will add emoji after preceeding emoji with space between them', function () {
        var text = 'Hi 😄 :wave:';
        expect(EmojiUtils.replaceEmojis(text).text).toBe('Hi 😄 👋 ');
    });
    ...
    it('suggests emojis when typing emojis prefix after colon, preceeded by another emoji ', function () {
        var text = 'Hi :ok: :coffin';
        expect(EmojiUtils.suggestEmojis(text, 'en')).toEqual([{ code: '⚰️', name: 'coffin' }]);
    });
