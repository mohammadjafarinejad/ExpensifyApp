...
describe('TransactionPreviewUtils', function () {
    ...
    describe('getTransactionPreviewTextAndTranslationPaths', function () {
        ...
        it('returns correct receipt error message when the transaction has receipt error', function () {
            var functionArgs = __assign(__assign({}, basicProps), { transaction: __assign(__assign({}, basicProps.transaction), { errors: {
                        error1: {
                    ...
                    } }), originalTransaction: undefined, shouldShowRBR: true });
            var result = (0, TransactionPreviewUtils_1.getTransactionPreviewTextAndTranslationPaths)(functionArgs);
        ...
        it('should display the correct amount for a bill split transaction', function () {
            var functionArgs = __assign(__assign({}, basicProps), { isBillSplit: true });
            var result = (0, TransactionPreviewUtils_1.getTransactionPreviewTextAndTranslationPaths)(functionArgs);
            expect(result.displayAmountText.text).toEqual('$1.00');
        });
        it('should display the correct amount for a bill split transaction after updating the amount', function () {
            var functionArgs = __assign(__assign({}, basicProps), { isBillSplit: true, transaction: __assign(__assign({}, basicProps.transaction), { modifiedAmount: 50 }) });
            var result = (0, TransactionPreviewUtils_1.getTransactionPreviewTextAndTranslationPaths)(functionArgs);
    ...
    describe('createTransactionPreviewConditionals', function () {
        ...
        it('should determine RBR visibility according to whether there is a receipt error', function () {
            var functionArgs = __assign(__assign({}, basicProps), { transaction: __assign(__assign({}, basicProps.transaction), { errors: {
                        error1: {
                            error: CONST_1.default.IOU.RECEIPT_ERROR,
                            source: 'source.com',
    ...
    describe('getViolationTranslatePath', function () {
        ...
        test('returns translationPath when there is at least one violation and transaction is on hold', function () {
            expect((0, TransactionPreviewUtils_1.getViolationTranslatePath)(mockViolations(1), false, message, true, false)).toEqual(reviewRequired);
        });
        test('returns translationPath if violation message is too long', function () {
            expect((0, TransactionPreviewUtils_1.getViolationTranslatePath)(mockViolations(1), false, longMessage, false, false)).toEqual(reviewRequired);
        });
        test('returns translationPath when there are multiple violations', function () {
            expect((0, TransactionPreviewUtils_1.getViolationTranslatePath)(mockViolations(2), false, message, false, false)).toEqual(reviewRequired);
        });
        test('returns translationPath when there is at least one violation and there are field errors', function () {
            expect((0, TransactionPreviewUtils_1.getViolationTranslatePath)(mockViolations(1), true, message, false, false)).toEqual(reviewRequired);
        });
        test('returns text when there are no violations, no hold, no field errors, and message is short', function () {
            expect((0, TransactionPreviewUtils_1.getViolationTranslatePath)(mockViolations(0), false, message, false, false)).toEqual({ text: message });
        });
        test('returns translationPath when there are no violations but message is too long', function () {
            expect((0, TransactionPreviewUtils_1.getViolationTranslatePath)(mockViolations(0), false, longMessage, false, false)).toEqual(reviewRequired);
        });
    });
    describe('getUniqueActionErrorsForTransaction', function () {
