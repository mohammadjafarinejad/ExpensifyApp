...
var UnreadIndicatorUpdater_1 = require("../../src/libs/UnreadIndicatorUpdater");
    ...
    describe('should return correct number of unread reports', function () {
        it('given last read time < last visible action created', function () {
            ...
            TestHelper.setPersonalDetails(TEST_USER_LOGIN, TEST_USER_ACCOUNT_ID).then(function () {
                expect(UnreadIndicatorUpdater_1.default.getUnreadReportsForUnreadIndicator(reportsToBeUsed, '3', undefined).length).toBe(2);
            });
        });
        it('given some reports are incomplete', function () {
            ...
            expect(UnreadIndicatorUpdater_1.default.getUnreadReportsForUnreadIndicator(reportsToBeUsed, '3', undefined).length).toBe(0);
        });
        it('given notification preference of some reports is hidden', function () {
            ...
            TestHelper.setPersonalDetails(TEST_USER_LOGIN, TEST_USER_ACCOUNT_ID).then(function () {
                expect(UnreadIndicatorUpdater_1.default.getUnreadReportsForUnreadIndicator(reportsToBeUsed, '3', undefined).length).toBe(1);
            });
        });
    });
});
